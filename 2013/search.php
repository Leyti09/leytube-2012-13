    ...
    <div id="page-container">
        <div id="page" class="  home  clearfix"><div id="guide">        <div id="guide-container" class="">
                ...
                <div class="branded-page-v2-container  branded-page-v2-secondary-column-hidden">
                    <div style="padding-top: 20px; padding-left: 200px;" class="branded-page-v2-col-container clearfix">
                    <ul id="watch-related" class="video-list">
                    <?php
                        $stmt = $conn->prepare("SELECT * FROM `users` WHERE username LIKE ? ORDER BY id DESC LIMIT 3");
                        ...
                        <!-- VIDEO START -->
                        <li class="video-list-item related-list-item"><a href="/user?n=<?php echo $row['username']; ?>" class="related-video yt-uix-contextlink  yt-uix-sessionlink" data-sessionlink="feature=related&amp;ei=n-PiUdfCIcfckwLEyoDACw&amp;ved=CAMQzRooAA">
                        <span class="ux-thumb-wrap contains-addto " style="position: relative; left: 13px;">    
                        <span class="video-thumb  yt-thumb yt-thumb-120" style="width: 90px;height: 90px;">
                            <span class="yt-thumb-default" style="height: 25px;">
                                <span class="yt-thumb-clip">
                                    <span class="yt-thumb-clip-inner">
                                        <img style="width: 90px; height: 90px;" alt="" src="/dynamic/pfp/<?php echo $row['pfp']; ?>" data-thumb="/thumbs/" data-group-key="thumb-group-0" width="120">
                                        <span class="vertical-align"></span>
                                    </span>
                                </span>
                            </span>
                            ...
                            </span><span dir="ltr" class="title" style="position: relative; left: 25px;"><?php echo $row['username']; ?></span>    
                            <span class="stat view-count" style="position: relative; left: 25px;">
                                <?php echo getSubscribers($row['username'], $conn); ?> subscribers
                                </span>
                            </a>
                        </li>
                        <!-- VIDEO END -->
                        <?php } ?>
                        <?php
                        $stmt = $conn->prepare("SELECT * FROM `playlists` WHERE title LIKE ? ORDER BY id DESC LIMIT 3");
                        $stmt->bind_param('s', $search);
                        $search = "%" . htmlspecialchars($_GET['search_query']) . "%";
                    ...
                    <span dir="ltr" class="title" title="YouTube Mix"><?php echo htmlspecialchars($row['title']); ?></span></a>
                        ...
                        <?php } ?>
                    <?php
                        $stmt = $conn->prepare("SELECT * FROM `videos` WHERE title LIKE ? AND visibility = 'v' ORDER BY id DESC");
                        $stmt->bind_param('s', $search);
                        $search = "%" . htmlspecialchars($_GET['search_query']) . "%";
